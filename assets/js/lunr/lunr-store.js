var store = [{
        ...
        "title": "検証誤差と汎化誤差",
        "excerpt":"機械学習では、train-valid-test分割 1 という方法が良く用いられる。 イメージを持つ：代表選抜と本番での実力 機械学習の議論をする前に、ちょっとイメージを持っておこう。 ******** 100m走の選手選抜を考えてみよう。とある高校で、以下のような実力伯仲な5人の100m走者がいるとする。彼らは調子によって \\(\\pm0.5\\) 秒のタイムのぶれがある。 走者 平均タイム 山田 11.20 秒 佐藤 11.21 秒 田中 11.17 秒 橋本 11.13 秒 加藤 11.15 秒 この5名から高校の代表選手を1名決めるために、タイム計測を行うことを考えてみよう。 ******** import numpy as np np.random.seed(0) # 毎回同じ結果が得られるように固定 # 表の情報を記入 name = [\"山田\", \"佐藤\", \"田中\", \"橋本\", \"加藤\"] ave_time = np.array([11.20, 11.21, 11.17, 11.13,...","categories": ["データサイエンス・機械学習"],
        "tags": ["教師あり学習","予測誤差"],
